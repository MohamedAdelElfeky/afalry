<x-default-layout>
                ...
                <table class="table table-row-dashed table-sm text-center gs-0 gy-4">
                    <thead>
                        <tr class="fw-bold text-muted">
                            ...
                            <th class="min-w-150px">{{ __('lang.deleg_value') }}</th>
                            <th class="min-w-150px">{{ __('lang.com_value') }}</th>
                            <th class="min-w-150px">{{ __('lang.del_value') }}</th>
                    ...
                    <nav role="navigation" aria-label="Pagination Navigation">
                        <div class="flex items-center justify-between">
                            ...
                            @foreach ($cities->getUrlRange(max(1, $cities->currentPage() - 1), min($cities->lastPage(), $cities->currentPage() + 1)) as $page => $url)
                                <a href="{{ $url }}"
                                    class="btn btn-outline-secondary {{ $page == $cities->currentPage() ? 'active' : '' }}">{{ $page }}</a>
                            @endforeach
    ...
    @include('pages/dashboards/cities/add')
