<x-default-layout>
    ...
    <div class="card mb-5 mb-xl-8">
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bold fs-3 mb-1">{{ __('lang.settings') }}</span>
            </h3>
                ...
                <table class="table table-row-dashed table-sm text-center gs-0 gy-4">
                    <thead>
                        <tr class="fw-bold text-muted">
                            ...
                            <th class="min-w-150px">{{ __('lang.key') }}</th>
                            <th class="min-w-150px">{{ __('lang.value') }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end flexpca-shrink-0">
                                        ...
                                        @include('pages/dashboards/settings/edit')
                ...
                <div class="pagination justify-content-center">
                    <nav role="navigation" aria-label="Pagination Navigation">
                        <div class="flex items-center justify-between">
                            ...
                            {{-- <a href="{{ $settings->url(1) }}" class="btn btn-outline-secondary">First</a> --}}
                            <a href="{{ $settings->previousPageUrl() }}" class="btn btn-outline-secondary">«
                                {{ __('lang.previous') }}</a>

                            @foreach ($settings->getUrlRange(1, $settings->lastPage()) as $page => $url)
                                <a href="{{ $url }}"
                                    class="btn btn-outline-secondary {{ $page == $settings->currentPage() ? 'active' : '' }}">{{ $loop->iteration }}</a>
                            @endforeach

                            <a href="{{ $settings->nextPageUrl() }}" class="btn btn-outline-secondary">
                                {{ __('lang.next') }} »</a>
                            {{-- <a href="{{ $settings->url($settings->lastPage()) }}"
    ...
    @include('pages/dashboards/settings/add')
